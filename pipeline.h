...
struct Pipeline<void>
{
    ...
    */
    template<typename ReturnType>
    ForkedPipeline<ReturnType> fork(std::function<ReturnType()> action, size_t worker=0, uint64_t deadline=0)
    {
        join_semaphore_t* join_sem = new join_semaphore_t(0);
        ReturnType* result = new ReturnType();
        task.then([=] () { *result = action(); }, worker, deadline);
    ...
    */
    template<typename ReturnType>
    ForkedPipeline<ReturnType> forkAbsolute(std::function<ReturnType()> action, size_t worker=0, uint64_t deadline=0)
    {
        join_semaphore_t* join_sem = new join_semaphore_t(0);
        ReturnType* result = new ReturnType();
        task.thenAbsolute([=] () { *result = action(); }, worker, deadline);
        return ForkedPipeline<ReturnType>(std::move(task), result, join_sem);
    }
...
struct Pipeline
{
    ...
    */
    template<typename ReturnType>
    ForkedPipeline<ReturnType> fork(std::function<ReturnType(PrevReturn)> action, size_t worker=0, uint64_t deadline=0)
    {
        join_semaphore_t* join_sem = new join_semaphore_t(0);
        ReturnType* result = new ReturnType();
        task.then([=] () { *result = action(*prev_result); delete prev_result; }, worker, deadline);
    ...
    */
    template<typename ReturnType>
    ForkedPipeline<ReturnType> forkAbsolute(std::function<ReturnType(PrevReturn)> action, size_t worker=0, uint64_t deadline=0)
    {
        join_semaphore_t* join_sem = new join_semaphore_t(0);
        ReturnType* result = new ReturnType();
        task.thenAbsolute([=] () { *result = action(*prev_result); delete prev_result; }, worker, deadline);
        return ForkedPipeline<ReturnType>(std::move(task), result, join_sem);
    }
